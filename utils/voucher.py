...
def save_voucher(voucher, ip_address=None):
    """Save voucher to the log file
    
    Args:
        voucher (dict or str): The voucher data to save or a voucher code
        ip_address (str, optional): The IP address of the user
    """
    # Handle case where voucher is a string (voucher code)
    if isinstance(voucher, str):
        code = voucher
        # Try to get existing voucher
        existing_voucher = get_voucher(code)
        if existing_voucher:
            voucher = existing_voucher
        else:
            # Create a new voucher with minimal info
            voucher = {
                'code': code,
    ...
    if not isinstance(voucher, dict):
        raise TypeError("Voucher must be a dictionary or a string (voucher code)")
    ...
    # Add IP address to voucher if provided
    if ip_address and 'ip_address' not in voucher:
